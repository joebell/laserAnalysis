function I = plotMutualInformationByThisStateTransition(expList)
...
I = zeros(3,3, size(timeLags,2));
% For each time lag
for fromTargetState = 1:3
for toTargetState = 1:3
    for timeLagN = 1:size(timeLags,2);  
        ...
        for expNn = 1:size(expList,2)
            ...
            for laneN = useLanes
                ...
                nextSequence  = [stateSequence(2:end);stateSequence(end)];
                targetSequence = ((stateSequence == fromTargetState)&...
                    (nextSequence == toTargetState)) + 1;
                nowSequence = targetSequence((1+timeLag):end);
        ...
        if (timeLag == 0)
            for nowState = 1:2
                ...
                I(toTargetState,fromTargetState,timeLagN) = I(toTargetState,fromTargetState,timeLagN) + marginalI;
            end
        else
            for nowState = 1:2
                for prevState = 1:3
...
for fromTargetState = 1:3
    ...
    plotColor = pretty( (fromTargetState - 1)*3 + toTargetState);
    % Plot it
    plot(-timeLags'./10, squeeze(I(fromTargetState,toTargetState,:)),'Color',plotColor); hold on;
    line([-timeLags(1)./10,-timeLags(end)./10],[1 1]*I(fromTargetState,toTargetState,1),...
